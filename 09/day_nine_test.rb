...
class HeightmapTest < MiniTest::Test
  ...
  def test_find_basins
    ...
    assert(4, found_basins.length)

    assert(3, found_basins[[0, 1]].length)
    assert(9, found_basins[[0, 9]].length)
    assert(14, found_basins[[2, 2]].length)
    assert(9, found_basins[[4, 6]].length)
  end
  ...
  def test_get_risk_levels
    ...
    assert(4, low_points.length)
    assert(low_points.value?(2))
    assert(low_points.value?(1))
    assert(low_points.value?(6)) # Recorded twice.
  end

  def test_total_risk_level
    assert(15, @heightmap.total_risk_level)
  end
end
