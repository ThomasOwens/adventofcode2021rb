...
class DiagnosticReport
  ...
  def parse_entry(entry)
    entry.chars.each_with_index do |value, index|
      if @report.key?(index)
        if @report[index].key?(value.to_i)
          @report[index][value.to_i] += 1
        else
          @report[index][value.to_i] = 1
        end
      else
  ...
  def gamma_rate
    most_common_bits = Array.new(@report.keys.length)

    @report.keys.each_with_index do |_, index|
      most_common_bits[index] = if @report[index][0] > @report[index][1]
                                  0
                                else
                                  1
                                end
  ...
  def epsilon_rate
    least_common_bits = Array.new(@report.keys.length)

    @report.keys.each_with_index do |_, index|
      least_common_bits[index] = if @report[index][0] < @report[index][1]
                                   0
                                 else
                                   1
                                 end
